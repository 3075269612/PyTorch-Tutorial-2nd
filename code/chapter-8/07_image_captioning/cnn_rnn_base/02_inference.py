...
def visualize_att(image_path, seq, alphas, rev_word_map, smooth=True, outdir=None):
    """
    ...
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    path_out = os.path.join(outdir, str(image_path.name))
    ...
    for t in range(len(words)):
        ...
        alpha_ = 0 if t == 0 else 0.8
        plt.imshow(alpha, alpha=alpha_)
...
if __name__ == '__main__':
    ...
    if path.is_dir():
        path_list_ = path.glob("*.jpg")
        path_list.extend(path_list_)

    for path_img in tqdm(path_list):
        img = cv2.imread(str(path_img), flags=1)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        # Encode, decode with attention and beam search
        seq, alphas = caption_image_beam_search(encoder, decoder, img, word_map, args.beam_size)
        alphas = torch.FloatTensor(alphas)
        # Visualize caption and attention of best sequence
        visualize_att(path_img, seq, alphas, rev_word_map, args.smooth, out_dir)
